...
random_num=Math.round((Math.random()*quick_draw_data_set.length)+1);
...
function setup(){
    ...
    canvas.position(620,550);
    canvas.mouseReleased(classifyCanvas);
    text_to_speech=window.speechSynthesis;
}
...
function gotResult(error,results){
    if(error){
        ...
        console.log(results);
        drawn_sketch=results[2].label;
        document.getElementById("drawing").innerHTML="Your Sketch:  "+results[2].label;
        document.getElementById("confidence").innerHTML="Confidence:  "+Math.round(results[1].confidence*100)+"%";
        utterThis=new SpeechSynthesisUtterance(results[0].label);
        text_to_speech.speak(utterThis);
    }
}
